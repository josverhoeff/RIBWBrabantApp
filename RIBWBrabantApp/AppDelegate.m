...
#define kNewsItemCodeKey    @"NewsItemCode"
...
@property (nonatomic, strong) NieuwsItem *singleNewPostedNewsItem;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [[NSUserDefaults standardUserDefaults] registerDefaults:@{ kSetting_ResetApp:@NO,
                                                               kSetting_ReadNewsItems:@[],
                                                               kSetting_UnreadNewsItemsCount:@(0),
                                                               kSetting_MapViewType:@(1),
                                                               kSetting_HideAlertForExternalURL:@NO,
                                                               kSetting_TimestampForLastNewsDownload:@"",
...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:kSetting_ResetApp]) {
        DLog(@"App settings will be cleared.");
        
        [[NSURLCache sharedURLCache] removeAllCachedResponses];
        
        // Also reset defaults in Settings bundle
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_ResetApp];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_ReadNewsItems];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_DownloadedNewsItems];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_UnreadNewsItemsCount];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_MapViewType];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_HideAlertForExternalURL];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_TimestampForLastNewsDownload];
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kSetting_ShowNewsItemsAsNotifications];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    
    // Update the unread news items badge count (look for it in the UserDefaults)
    NSInteger unreadNewsItemCount = [[NSUserDefaults standardUserDefaults] integerForKey:kSetting_UnreadNewsItemsCount];
    
    // App has become active, so alert dialogs are permitted
    self.shouldShowAlertView = YES;
    
    // Check for new  news items
    NieuwslijstTableViewController *newsItemListController;
...
{
    // The user tapped a local notification (for a recently downloaded news item) generated by the  app.
    NieuwslijstTableViewController *newsItemListController;
    
    // Display the News tab right away
    if (self.window && self.window.rootViewController) {
        RootViewController *rootVC = (RootViewController *)self.window.rootViewController;
        if ([rootVC.viewControllers count]>2) {
    ...
    if (newsItemListController && notification.userInfo && [notification.userInfo objectForKey:kNewsItemCodeKey]) {
        NSString *newsItemID = [notification.userInfo objectForKey:kNewsItemCodeKey];
...
- (void)application:(UIApplication *)application performFetchWithCompletionHandler:(void (^)(UIBackgroundFetchResult result))completionHandler
{
    ...
    [self checkPermissionsForLocalNotificationOfType:UIUserNotificationTypeAlert thenScheduleBlock:^{
        ...
        if (strongSelf) {
            ...
            NSDate *timestamp = [[NSUserDefaults standardUserDefaults] objectForKey:kSetting_TimestampForLastNewsDownload];
                ...
                [strongSelf refreshNewsItemListForController:nil withCompletionHandler:^{
                    ...
                    if (strongSelf.numberOfNewPostedNewsItems >= 1) {
                        ...
                        if (strongSelf.singleNewPostedNewsItem) {
                            ...
                            localNotification.userInfo = @{ kNewsItemCodeKey : strongSelf.singleNewPostedNewsItem.identifier };
                        } else {
...
- (void)refreshNewsItemListForController:(NieuwslijstTableViewController *)newsItemListController withCompletionHandler:(void (^)())completionHandler
{
    ...
    self.backgroundFetchNewsItemRequest = [manager GET:[HTTPRequestFactory urlForNewsItemList] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSArray *previouslyDownloadedNewsItems = [[NSUserDefaults standardUserDefaults] arrayForKey:kSetting_DownloadedNewsItems];
        if (!previouslyDownloadedNewsItems)
            previouslyDownloadedNewsItems = @[];
        NSArray *readNewsItems = [[NSUserDefaults standardUserDefaults] arrayForKey:kSetting_ReadNewsItems];
        if (!readNewsItems)
            readNewsItems = @[];
        ...
        for (id jsonObject in jsonResponseArray) {
            NieuwsItem *newsItem = [MTLJSONAdapter modelOfClass:[NieuwsItem class] fromJSONDictionary:jsonObject error:&error];
            if (!newsItem) {
                DLog(@"Error converting JSON news item info: %@, %@", [error localizedDescription], [error userInfo]);
            } else {
        ...
        if (weakSelf.window && weakSelf.window.rootViewController) {
            ...
            RootViewController *rootViewController = (RootViewController *)weakSelf.window.rootViewController;
        ...
        if (downloadedNewsItems) {
            ...
            [[NSUserDefaults standardUserDefaults] setObject:downloadedNewsItems forKey:kSetting_DownloadedNewsItems];
        ...
        DLog(@"Error downloading news items list info: %@, %@", [error localizedDescription], [error userInfo]);
...
- (void)checkPermissionsForLocalNotificationOfType:(UIUserNotificationType)type thenScheduleBlock:(void (^)())notificationBlock
{
    ...
    if ([[UIApplication sharedApplication] respondsToSelector:@selector(currentUserNotificationSettings)]) {
        ...
        NSNumber *showNewsItemsAsNotifications = [[NSUserDefaults standardUserDefaults] valueForKey:kSetting_ShowNewsItemsAsNotifications];
        ...
        if ([showNewsItemsAsNotifications integerValue] == 1) {
            ...
            if (self.shouldShowAlertView && (!self.alertViewActive)) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Meldingen" message:@"De app wil u graag op de hoogte houden van nieuws.\nWilt u daarvoor meldingen inschakelen?" delegate:self cancelButtonTitle:@"Nee" otherButtonTitles:@"Ja", nil];
                alert.tag = 2;
                [alert show];
...
- (void)openExternalURL:(NSURL *)url
{
    ...
    BOOL showAlert = (![[NSUserDefaults standardUserDefaults] boolForKey:kSetting_HideAlertForExternalURL]);
    ...
    switch (alertView.tag) {
        case 1: {
            ...
            if (self.externalURL && (buttonIndex != alertView.cancelButtonIndex)) {
                ...
                if ([[alertView buttonTitleAtIndex:buttonIndex] isEqualToString:@"Altijd"]) {
                    [[NSUserDefaults standardUserDefaults] setBool:YES forKey:kSetting_HideAlertForExternalURL];
                    [[NSUserDefaults standardUserDefaults] synchronize];
                }
        ...
        case 2: {
            ...
            if (buttonIndex != alertView.cancelButtonIndex) {
                ...
                [[NSUserDefaults standardUserDefaults] setValue:@(1) forKey:kSetting_ShowNewsItemsAsNotifications];
                [[NSUserDefaults standardUserDefaults] synchronize];
                ...
                // Permission denied
                [[NSUserDefaults standardUserDefaults] setValue:@(0) forKey:kSetting_ShowNewsItemsAsNotifications];
                [[NSUserDefaults standardUserDefaults] synchronize];
